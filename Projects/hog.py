...
def roll_dice(num_rolls, dice=six_sided):
    ...
    counter, result_from_roll, total, sowSad_Case  = 1, 0, 0, 0

    while (counter <= num_rolls):
        result_from_roll = dice()

        if (result_from_roll == 1):
            sowSad_Case = 1
        else:
            total += result_from_roll
        counter += 1
...
def tail_points(opponent_score):
    ...
    result, pos_Ones, pos_Tens = 0, 0, 0

    pos_Ones += opponent_score % 10
    pos_Tens = opponent_score // 10 % 10
    result =  (2 * abs(pos_Tens - pos_Ones)) +1
...
def perfect_square(score):
    ...
    number = 1
    while (number * number <= score):
        if ((score % number == 0) and (score // number == number)):
            return True
        number = number + 1
    return False
...
def next_perfect_square(score):
    ...
    sqrt_Value = score ** 0.5
    next_PerfectSquare_Value = int((sqrt_Value + 1) ** 2)

    return next_PerfectSquare_Value
...
def is_always_roll(strategy, goal=GOAL):
    ...
    same_Value = strategy(0,0)
    opponent_score = 0
    while (opponent_score <= 99):
        my_score = 0
        while (my_score <= 99):
            if (strategy(my_score, opponent_score) != same_Value):
                return False
            my_score += 1
        opponent_score += 1
    return True
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    opp_points = tail_points(opponent_score)
    total = score + opp_points
    if (perfect_square(total) == True):
         total = next_perfect_square(total)
